...
/*********************************************************************************/
/*********************************************************************************/
class Instacia : public pp::Instance,
                        public pp::Graphics3DClient {
 public:
  Instacia(PP_Instance instance, pp::Module* module);
  virtual ~Instacia();
 ...
 private:
  ...
  pp::CompletionCallbackFactory<Instacia> callback_factory_;
...
//Setar o tamanho do app
void Instacia::DidChangeView(
...
void Instacia::HandleMessage(const pp::Var& var_message) {
  ...
  if (command == "init") {
    ...
    std::string str = "teste";//video_track_.GetId();
    pp::Var var_reply(str);
...
void Instacia::Render() {
  ...
  pp::CompletionCallback cb = callback_factory_.NewCallback(
      &Instacia::PaintFinished);
  context_->SwapBuffers(cb);
}

void Instacia::PaintFinished(int32_t result) {
  is_painting_ = false;
  if (needs_paint_)
    Render();
}

GLuint Instacia::CreateTexture(
...
void Instacia::ConfigureTrack() {
  ...
  video_track_.Configure(attrib_list, callback_factory_.NewCallback(
        &Instacia::OnConfigure));
}

void Instacia::OnConfigure(int32_t result) {
  video_track_.GetFrame(callback_factory_.NewCallbackWithOutput(
      &Instacia::OnGetFrame));
}

void Instacia::OnGetFrame(
...
pp::Instance* Modulo::CreateInstance(PP_Instance instance) {
  return new Instacia(instance, this);
}
